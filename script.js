...
// --- HELPER TO FIX DD/MM/YYYY DATES & LOCK TIMEZONE ---
function safeParseDate(dateStr) {
  ...
  try {
    ...
    if (dParts.length === 3) {
      ...
      let isoString = `${dParts[2]}-${dParts[1]}-${dParts[0]}T${tParts[0]}:${tParts[1]}:${tParts[2] || '00'}+05:30`;
...
// ── CLOCK & DYNAMIC DATE LIVE UPDATE ──
function updateClock(){ 
  const now = new Date();
  document.getElementById('clock').textContent = now.toLocaleTimeString('en-IN',{hour12:true}); 
  
  // This ensures the date at the top left flips over at midnight!
...
function showPage(id, btn){
  ...
  if (id === 'dashboard') { 
    const now = new Date();
    document.getElementById('page-sub').textContent = `IoT Lab — ${now.toLocaleDateString('en-IN',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}`; 
  }
...
function updateDashboardStats() {
  // "Currently inside" doesn't reset (in case a student stays over midnight)
  const presentCount = attendanceData.filter(r=>r.status==='present').length;
  
  // Stats specifically calculated for TODAY ONLY
  const exitCountToday = attendanceData.filter(r=>r.status==='left' && isToday(r.outTime)).length;
  
  let scansToday = 0;
  attendanceData.forEach(r => {
    if(isToday(r.inTime)) scansToday++;
    if(isToday(r.outTime)) scansToday++;
  });
...
function renderActivityList(){
  const events = [];
  attendanceData.forEach(r=>{
    // Filter Recent Activity to only show events that happened TODAY
    if(r.inTime && isToday(r.inTime)) events.push({name:r.name, roll:r.roll, time:r.inTime, type:'IN'});
    if(r.outTime && isToday(r.outTime)) events.push({name:r.name, roll:r.roll, time:r.outTime, type:'OUT'});
  });
  ...
  // Show a message if no one has scanned in yet today
  if(top.length === 0) {
     list.innerHTML = `<div style="padding:20px; text-align:center; color:#94a3b8; font-size:13px; font-weight:500;">No activity yet today.</div>`;
     return;
  }
